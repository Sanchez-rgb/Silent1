...
class XiaohongshuMockAPI:
    ...
    def _generate_random_comments(self):
        comments = []
        positive_texts = ['太赞了！', '好喜欢！', '太棒了！', '学到了！', '种草了！', '太好看了！', '已收藏！']
        neutral_texts = ['还行吧', '一般般', '看看', '嗯', '了解了', '可以', '哦']
        negative_texts = ['不太好', '一般', '不喜欢', '有点失望', '不怎么样', '一般般']
...
def run_scheduled_task():
    ...
    print(f"本次处理评论数: {total_comments}")
    print(f"数据库总评论数: {total_in_db}")
...
if __name__ == "__main__":
    ...
    print("\n4. 设置定时任务（每天凌晨 02:00 执行）")
    schedule.every().day.at("02:00").do(run_scheduled_task)
    print("定时任务已设置\n")

    print("=" * 60)
    print("系统演示完成！")
    print("使用说明：")
    print("  - 运行 'python complete_scheduler.py' 即可启动定时任务")
    print("  - 程序会在每天 02:00 自动执行")
    print("  - 按 Ctrl+C 停止程序")
    print("=" * 60)
